...
function handlers.onReplayInfiniteTower()
    local args = {};
    client_to_server:WaitForChild('request_start_infinite_towner_from_game'):InvokeServer(unpack(args));
end

function handlers.onDeleteUniqueItems(items)
    local args = {
        [1] = items
    }
    client_to_server:WaitForChild('delete_unique_items'):InvokeServer(unpack(args));
end

function handlers.onUseActive(model)
    local args = {
        [1] = model;
    }
    client_to_server:WaitForChild('use_active_attack'):InvokeServer(unpack(args));
end
