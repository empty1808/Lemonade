...
local function SetUUID(instance)
...
do
	...
	function section:addTextLabel(title, text)
		local TextLabel = utility:Create("Frame", {
			Name = "TextLabel",
			Parent = self.container,
			BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 0, 30),
	...
	function section:addSelectDropdown(title, list, selected, callback)
		local SelectDropdown = utility:Create("Frame", {
			Name = "SelectDropdown",
			Parent = self.container,
			BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 0, 30),
			...
			utility:Create("Frame", {
				...
				utility:Create("ImageLabel", {
					...
					utility:Create("TextLabel", {
						...
						ZIndex = 3,
						Font = Enum.Font.Gotham,
						Text = ((selected and (#selected > 0)) and table.concat(selected, ', ') or 'nil'),
						TextColor3 = themes.TextColor,
						TextSize = 12,
						TextTransparency = 0.10000000149012,
						TextXAlignment = Enum.TextXAlignment.Left
		...
		table.insert(self.modules, SelectDropdown);
		local Selected = SelectDropdown.Containers.Selected;

		Selected.Button.MouseButton1Click:Connect(function()
			if Selected.Button.Rotation == 0 then
				self:updateSelectDropdown(SelectDropdown, nil, list, selected, callback)
			else
				self:updateSelectDropdown(SelectDropdown, nil, nil, selected, callback)
			end
		end)

		SelectDropdown:GetPropertyChangedSignal("Size"):Connect(function()
			self:Resize()
		end)
	...
	function section:updateTextLabel(TextLabel, title, text)
		...
		if (title) then
			TextLabel.Title.Value.Text = title;
		end

		if (text) then
			TextLabel.Text.Value.Text = text;
		end
	end
	...
	function section:updateSelectDropdown(SelectDropdown, title, list, selected, callback)
		SelectDropdown = self:getModule(SelectDropdown);

		if (title) then
			SelectDropdown.Title.TextLabel.Text = title;
		end

		if (selected and (#selected > 0)) then
			SelectDropdown.Containers.Selected.TextLabel.Text = table.concat(selected, ', ');
		else 
			SelectDropdown.Containers.Selected.TextLabel.Text = 'nil';
		end

		local entries = 0;

		utility:Pop(SelectDropdown.Containers.Selected, 10);

		for i, button in pairs (SelectDropdown.Containers.List.Frame:GetChildren()) do
		...
		for i, value in pairs (list or {}) do
			...
			for i, v in pairs (selected or {}) do
				if (value == v) then
					isSelected = true;
					index = i;
					break;
			...
			local button = utility:Create("ImageButton", {
				Parent = SelectDropdown.Containers.List.Frame,
				BackgroundTransparency = 1,
				BorderSizePixel = 0,
				Size = UDim2.new(1, 0, 0, 30),
				ZIndex = 2,
				Image = "rbxassetid://5028857472",
				ImageColor3 = (isSelected and themes.SelectDarkContrast or themes.DarkContrast),
				ScaleType = Enum.ScaleType.Slice,
				SliceCenter = Rect.new(2, 2, 298, 298)
			}, {
				utility:Create("TextLabel", {
			...
			button.MouseButton1Click:Connect(function()
				if (isSelected) then
					...
					table.sort(selected);
					self:updateSelectDropdown(SelectDropdown, title, list, selected, callback);	
				else
				...
				if (callback) then
					callback(selected, function(...)
						self:updateSelectDropdown(SelectDropdown, ...);
					end)
				end
			end)
		...
		local frame = SelectDropdown.Containers.List.Frame
		
		utility:Tween(SelectDropdown, {Size = UDim2.new(1, 0, 0, (entries == 0 and 30) or math.clamp(entries, 0, 3) * 34 + 38)}, 0.3)
		utility:Tween(SelectDropdown.Containers.Selected.Button, {Rotation = list and 180 or 0}, 0.3)
	...
	function section:updateDropdown(dropdown, title, list, selected, callback)
		dropdown = self:getModule(dropdown)
		if selected then
			local uuid = GetUUID(dropdown);
			Elements[uuid].Selected = selected;
			dropdown.Containers.Search.TextBox.Text = selected;
		end
		
		local entries = 0
		
		utility:Pop(dropdown.Containers.Search, 10)
		
		for i, button in pairs(dropdown.Containers.List.Frame:GetChildren()) do
			if button:IsA("ImageButton") then
				button:Destroy()
			end
		end
		...
		for i, value in pairs(list or {}) do
			local button = utility:Create("ImageButton", {
				Parent = dropdown.Containers.List.Frame,
				BackgroundTransparency = 1,
				BorderSizePixel = 0,
				Size = UDim2.new(1, 0, 0, 30),
				ZIndex = 2,
				Image = "rbxassetid://5028857472",
				ImageColor3 = ((value == selected) and themes.SelectDarkContrast or themes.DarkContrast),
				ScaleType = Enum.ScaleType.Slice,
				SliceCenter = Rect.new(2, 2, 298, 298)
			}, {
				utility:Create("TextLabel", {
			...
			button.MouseButton1Click:Connect(function()
				...
				self:updateDropdown(dropdown, title, nil, value, callback)
			end)
			
			entries = entries + 1
		end
		
		local frame = dropdown.Containers.List.Frame
		
		utility:Tween(dropdown, {Size = UDim2.new(1, 0, 0, (entries == 0 and 30) or math.clamp(entries, 0, 3) * 34 + 38)}, 0.3)
		utility:Tween(dropdown.Containers.Search.Button, {Rotation = list and 180 or 0}, 0.3)
